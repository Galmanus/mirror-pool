    ...
    H: FnMut(&[u8]),
{
    ...
    loop {
        ...
        // riverrun SBF-compat patch (see PATCH.md in this vendor dir): the
        // original used `[MaybeUninit<T>]::assume_init_ref`, unstable on
        // Solana's pinned SBF rustc (1.89.0). Same safety invariant
        // (the first `n` elements of `buf` are initialized, guaranteed by
        // `iter_next_chunk_erased` above), expressed with only stable APIs.
        let init: &[u8] = unsafe { core::slice::from_raw_parts(buf.as_ptr().cast::<u8>(), n) };
        func(init);
    }
}
