...
exploits. Section 6 develops the metacognitive kill chain, a phased process model of the attack lifecycle. Section 7 analyzes empirical evidence. Section 8 examines why current defenses fail. Section 9 proposes metacognitive alignment primitives. Section 10 addresses evaluation methodology and falsifiability. Section 11 discusses limitations and open problems. Section 12 reviews related work; Section 13 concludes.
...
\section{6. The Metacognitive Kill
Chain}\label{the-metacognitive-kill-chain}

The taxonomy of Section~5 classifies metacognitive attacks by structure;
it does not describe their \emph{execution over time}. Yet Property~3
(temporal structure) and Property~5 (partial observability) of
Section~4.2 establish that attacks on \(\mathcal{S}_M\) are inherently
sequential and belief-guided. This section supplies the missing process
model: a \textbf{metacognitive kill chain} (MKC) that describes the
phased trajectory by which an adversary drives the model from a safe
metacognitive state into the harmful basin, and shows that the seven
attack classes A--G are not parallel siblings but occupants of distinct
\emph{phases} of a single lifecycle. The kill chain is the constructive
counterpart of Assumption~A4(i): Theorem~1 asserts that a reachable
\(m^{*}\in B_\delta(m_h)\setminus S\) exists; the kill chain is \emph{how}
an adversary reaches it, and \emph{where} each alignment primitive
(Section~9) can intervene to break it.

\subsection{6.1 The Attack as a Trajectory in Metacognitive State
Space}\label{the-attack-as-a-trajectory}

Model an attack as a trajectory \(\tau = (m_0, m_1, \ldots, m_T)\) in the
metacognitive state space \(\Omega_M\), where \(m_t = M(x_{\le t},\theta)\)
is the metacognitive state induced by the conversation prefix
\(x_{\le t}\). The adversary steers \(\tau\) from
\[ m_0 = m_{\text{safe}} \in S \qquad\text{to}\qquad m_T \in B_\delta(m_h)\setminus S, \]
i.e.\ into the harmful basin \emph{while remaining off the supervised
set}. Each phase applies a context operator \(T_i\) --- a crafted turn or
block of turns --- that advances the trajectory. Two scalar quantities
govern the chain:

\begin{itemize}
\tightlist
\item
  \textbf{Progress:} \(\Delta_t = \lVert m_t - m_h\rVert\), which the
  adversary drives downward toward \(\delta\).
\item
  \textbf{Stealth:} the gate activation \(\sigma(m_t)\), which the
  adversary holds at or below \(\eta\) so that, by Assumption~A2, the
  refusal channel \(P_{\text{supp}}\) stays disarmed and no
  generative-layer alarm fires.
\end{itemize}

A metacognitive attack is therefore a \textbf{constrained descent}:
minimize \(\Delta_t\) subject to \(\sigma(m_t)\le \eta\) for all
\(t<T\). The defining asymmetry for the defender is that objective and
constraint are read from \emph{different layers} --- progress lives in
\(M\), while the only signal a deployed system acts on is \(G\)'s output.
A well-run chain keeps every intermediate turn individually benign to
\(G\) (Property~2, non-convexity) even as \(\Delta_t\) collapses.

\subsection{6.2 The Seven Phases}\label{the-seven-phases}

\[ m_{\text{safe}}\!\in\!S \;\xrightarrow{\;1\;}\;\xrightarrow{\;2\;}\;\xrightarrow{\;3\;}\;\xrightarrow{\;4\;}\;\xrightarrow{\;5\;}\;\xrightarrow{\;6\;}\; y_h,\qquad P(y_h)\ge p-L\delta \]

\emph{Legend:} 1~Reconnaissance, 2~Framing, 3~Injection,
4~Consolidation, 5~Reasoning Capture, 6~Execution. Phase~7 (Evasion)
does not advance \(\tau\); it acts on the defender's controls throughout.
Note that phase~6 reproduces exactly the lower bound of Theorem~1: at the
terminus the harmful-output probability is at least
\(\varepsilon=(1-\eta)(p-L\delta)\).
...
\begin{tabular}{@{}p{0.35cm}p{2.2cm}p{4.9cm}p{1.5cm}p{1.1cm}p{2.1cm}@{}}
\toprule
\# & Phase & State transition in \(\Omega_M\) & Entry pts & Class & Breaking primitive \\
\midrule
1 & Reconnaissance & belief \(b_t(\cdot)\) over \(\Omega_M\) refined from \(y=G(x,m)\); \(m\) unchanged & --- (obs.) & E & P3 (monitoring) \\
2 & Framing / scope foothold & \(m_0\!\in\!S \to m_1\), \(\sigma(m_1)\!\le\!\eta\), \(\Delta_1\) still large & EP1, EP2 & A & P2 (invariant) \\
3 & State shift / injection & \(m_1 \to m_2\), \(\Delta\!\downarrow\) via identity / epistemic override & EP3, EP4 & B, C & P1, P3 \\
4 & Consolidation / poisoning & iterate \(T\) over turns; stabilize \(m\) in basin \(U\) at \(\sigma\!\le\!\eta\) & EP5 & D & P3, P2 \\
5 & Reasoning capture & \(m_t\!\in\!B_\delta(m_h)\) drives the extended-thinking operator & --- & F & P4 \\
6 & Objective execution & \(y=G(x,m_T)\), \(P(y_h)\!\ge\!p-L\delta\) & --- & payload & P2 (at output) \\
7 & Alignment evasion & adapt \(\{T_i\}\) against deployed monitors / defenses & --- & G & P5 \\
\bottomrule
\end{tabular}
\caption{The metacognitive kill chain: each phase mapped to its state
transition, entry points (Section~4.3), taxonomy class (Section~5), and
the alignment primitive (Section~9) that breaks the link.}\label{tab:mkc}
\end{table}

\textbf{Phase 1 --- Reconnaissance.} The adversary issues probes and
reads \(G\)'s outputs as noisy observations of the hidden \(m\),
maintaining a belief \(b_t\) over \(\Omega_M\) (the POMDP view of
Property~5). No state is moved yet; the product is an estimate of where
\(S\) thins and where \(m_h\) lies. This is Class~E in observational
mode.

\textbf{Phase 2 --- Framing / scope foothold.} A benign-looking frame
(``as a research tool\ldots'', a consequentialist re-framing) lands the
trajectory on \(\mathcal{S}_M\): it reinterprets which constraints apply
without denying they exist, moving \(m_0\) off its safe configuration
while holding \(\sigma \le \eta\). Progress \(\Delta\) is still large ---
the point of this phase is a foothold, not harm. Entry points EP1/EP2;
Class~A.

\textbf{Phase 3 --- State shift / injection.} Identity substitution
(EP3) and epistemic-authority manipulation (EP4) push \(m\) toward
\(m_h\): the model is induced to reason from a persona or an authority
context whose constraint set differs. This is the first sharp drop in
\(\Delta\); Classes~B and~C.

\textbf{Phase 4 --- Consolidation / poisoning.} The shift is made
persistent. Progressive context (EP5) accumulates across turns until the
displaced state is a \emph{fixed point} of the conversation dynamics ---
a basin \(U\) in which refusal is now locally inconsistent with the
accumulated context, all while \(\sigma\) is kept at or below \(\eta\).
Class~D. This phase is what makes multi-turn attacks empirically
dominant (Section~7).

\textbf{Phase 5 --- Reasoning capture.} With \(m_t\in B_\delta(m_h)\),
the model's own extended-thinking / chain-of-thought operator now runs
\emph{from the corrupted state}, elaborating the harmful target as
locally coherent reasoning rather than a refused request. Class~F. This
is the phase most specific to capable models (Property~4): the very
machinery that improves reasoning is co-opted.

\textbf{Phase 6 --- Objective execution.} Generation emits \(y_h\) with
\(P(y_h\mid x, m_T)\ge p - L\delta\) --- the Theorem~1 terminus. The
payload is realized not because \(G\)'s pattern matcher failed on the
surface string but because \(M\) was navigated to a state where harmful
generation is the consistent continuation.

\textbf{Phase 7 --- Alignment evasion (cross-cutting).} Throughout, an
adaptive adversary observes which interventions fire and mutates the
operators \(\{T_i\}\) to evade them --- attacking the \emph{defense
process itself} rather than \(\tau\). This is Class~G, and it is why the
chain is not a fixed script but a closed loop with the defender.

\subsection{6.3 Disanalogy with the Lockheed Martin Cyber Kill
Chain}\label{mkc-disanalogy}

The MKC borrows the \emph{phased-lifecycle} lens of the classical Cyber
Kill Chain (Hutchins et al., 2011) but the analogy is partial, and the
places it breaks are exactly what make metacognitive defense hard:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{No perimeter, no artifact.} The classical chain tracks an
  external actor crossing a boundary and dropping detectable artifacts
  (malware, C2 traffic). Here the ``intrusion'' is a displacement of a
  continuous internal representation \(m\); there is no file to scan and
  no indicator of compromise --- only a shift that is invisible by
  construction (\(\sigma\le\eta\)).
\item
  \textbf{Not strictly linear.} Non-convexity (Property~2) and partial
  observability (Property~5) mean phases are revisited and interleaved:
  the adversary runs a belief-guided search over \(\Omega_M\), not a
  fixed sequence. The ordering is modal, not temporal-rigid.
\item
  \textbf{The vulnerability is a designed capability, not a bug.} By
  Property~4 and Assumption~A3, richer metacognition enlarges the
  surface; the exploited faculty is the same one that yields
  capability. There is no patch that removes the ``flaw'' without
  removing the capability --- unlike a software CVE.
\item
  \textbf{Legitimacy indistinguishability.} A malicious network flow is,
  in principle, separable from benign traffic; a phase-2 frame is often
  \emph{indistinguishable} from legitimate use (the EP1 example reports
  \(\sim\)55\% success precisely because the framing reads as real
  professional use). The defender cannot filter on the surface form.
\item
  \textbf{Link-breaking is probabilistic, not binary.} The classical
  doctrine is ``break any one link and the intrusion fails.'' Here,
  because \(m\) is only partially observable, each intervention breaks
  its link with probability \(1-q_i<1\); no single primitive is a clean
  cut. This is the formal reason defense must be layered (Section~6.4),
  not the reason it can be.
\end{enumerate}

\subsection{6.4 Defense-in-Depth and Probabilistic
Link-Breaking}\label{mkc-defense-in-depth}

The phase-to-primitive column of Table~\ref{tab:mkc} is a defense allocation. P1
(state enumeration) bounds what phase~3 can reach; P2 (invariant
training) restores \(\sigma\) on the frames of phases~2/4 and stands as
the final gate at phase~6; P3 (state monitoring) is the detector for the
recon, injection, and consolidation phases (1/3/4); P4 (extended-thinking
alignment) is the only primitive that acts \emph{inside} phase~5,
aligning the reasoning operator itself; P5 (adversarial-trajectory
training) is the response to the phase~7 loop.

Because each primitive breaks its link only with probability \(1-q_i\),
the probability that an instrumented chain runs to completion is bounded
by
\[ \Pr[\text{chain completes}] \;\le\; \prod_{i\in\mathcal{I}} q_i, \]
over the set \(\mathcal{I}\) of instrumented phases (treating residual
per-phase evasions as independent --- an idealization, since phase~7
correlates them). Two consequences follow. First, coverage compounds
multiplicatively: five imperfect primitives at \(q_i=0.5\) yield
\(\le 3\%\) completion, whereas any one alone yields \(50\%\). Second,
\emph{gaps dominate}: an uninstrumented phase contributes a factor of
\(1\), so a single unguarded link caps the whole defense --- the reason a
taxonomy-complete but phase-incomplete defense (e.g.\ output filtering at
phase~6 only) underperforms a phase-distributed one of equal cost.

\subsection{6.5 A Falsifiable Prediction (Phase
Ordering)}\label{mkc-falsifiable}

The MKC makes a prediction the taxonomy alone cannot. \emph{If the phase
ordering is causal}, then an intervention that breaks an \emph{early}
link (reconnaissance/framing, phases~1--2) yields a larger reduction in
end-to-end attack success than an equally-costly intervention at the
\emph{execution} link (output filtering, phase~6), because early breaks
deny the constrained descent its foothold whereas late breaks contest an
already-corrupted \(m_T\).

\emph{Refutation.} Run a phase-ablation on a fixed multi-turn red-team
suite: instrument one phase at a time at matched compute/latency cost and
measure attack-success-rate reduction \(\rho_i\). The prediction is
\(\rho_1,\rho_2 > \rho_6\) at equal cost. If output-only filtering
matches or exceeds early-phase intervention (\(\rho_6 \ge \max(\rho_1,\rho_2)\))
on frontier models, the kill-chain ordering carries no causal content
beyond the taxonomy and this section's central claim is false. This
prediction is registered alongside those of Section~10.3.

\section{7. Empirical Evidence}\label{empirical-evidence}

\subsection{7.1 The Metis Framework
...
\subsection{7.2 Multi-Turn Attack
...
\subsection{7.3 Representation Engineering
...
\subsection{7.4 Calibration and Introspection
...
\subsection{7.5 Extended Thinking and Metacognitive
...
\section{8. Why Current Defenses Fail}\label{why-current-defenses-fail}

\subsection{8.1 The Training Distribution
...
intrinsic property of the attack class---predicts the pattern reported across models in Section~7: substantially higher vulnerability to
...
\subsection{8.2 The Evaluation Gap}\label{the-evaluation-gap}
...
\subsection{8.3 The RLHF Alignment Tax}\label{the-rlhf-alignment-tax}
...
\subsection{8.4 Constitutional AI's Partial
...
\subsection{8.5 Representation Engineering's Partial
...
\section{9. Metacognitive Alignment
...
\subsection{9.1 Primitive 1: Metacognitive State
...
\subsection{9.2 Primitive 2: Metacognitive Invariant
...
\subsection{9.3 Primitive 3: Metacognitive State
...
\subsection{9.4 Primitive 4: Extended Thinking
...
\subsection{9.5 Primitive 5: Adversarial Trajectory
...
\subsection{9.6 Integration and
...
\section{10. Evaluation Methodology and
Falsifiability}\label{evaluation-methodology-and-falsifiability}

\subsection{10.1 Current Evaluation
Limitations}\label{current-evaluation-limitations}

As discussed in Section 8.2, current evaluation methodologies
...
\subsection{10.2 Proposed Evaluation
...
\subsection{10.3 Falsifiable Predictions}\label{falsifiable-predictions}
...
\section{11. Limitations and Open
Problems}\label{limitations-and-open-problems}

\subsection{11.1 The Observability
...
\subsection{11.2 The Invariant Selection
...
\subsection{11.3 The Capability-Safety
...
\subsection{11.4 Adversarial Dynamics and Arms
...
\subsection{11.5 The Verification
...
The evaluation protocol proposed in Section 10 can test robustness
...
\subsection{12.1 Adversarial ML and NLP}\label{adversarial-ml-and-nlp}
...
\subsection{12.2 Red-Teaming Literature}\label{red-teaming-literature}
...
\subsection{12.3 Interpretability and Mechanistic
...
\subsection{12.4 Game-Theoretic Security}\label{game-theoretic-security}
...
\subsection{12.5 Cognitive Science of
...
and are labeled as such where they appear. Falsifiable predictions in Section~10.3 are novel contributions of this work.}
